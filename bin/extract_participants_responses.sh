...
usage() {
  ...
  echo ""
  echo "Extracts participants from the JSON input by matching .participantId against the given regex filter."
}
...
for participant in $( jq -c '.participantId' $FILTERED | sort -u ); do
  ...
  echo "Extracting responses from participant $PARTICIPANT_ID"
  ...
  echo "     There are $count trials for this participant id."

  for i in $(seq 1 $count); do
    OUTPUT_PATH=${PARTICIPANT_DATA_PATH}/${PARTICIPANT_ID}_Responses_${i}.json

    echo "     Extracting $PARTICIPANT_ID response set # ${i} to $OUTPUT_PATH"
    sed "${i}q;d" $PARTDATA | jq . > "${OUTPUT_PATH}"
  done
